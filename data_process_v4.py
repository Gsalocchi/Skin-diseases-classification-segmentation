...
def get_transforms(image_size=(384, 384)):
    train_tf = transforms.Compose([
        # gentler random crop / zoom
        transforms.RandomResizedCrop(
            size=image_size,
            scale=(0.9, 1.0),
            ratio=(0.95, 1.05),
        ),
        # moderate rotation; avoid extreme artifacts
        transforms.RandomRotation(
            degrees=45,
            fill=0,   # ideally use approx background color if supported
        ),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomVerticalFlip(p=0.5),

        # conservative color jitter
        ...
        # optional: keep mild; maybe toggle off to see impact
